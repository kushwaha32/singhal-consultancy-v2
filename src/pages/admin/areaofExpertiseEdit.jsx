...
const AreaofexpertiseEdit = () => {
  ...
  const areaofexpertiseGet = useSelector((state) => state.areaofexpertiseData);
  const { areaofexpertiseAll } = areaofexpertiseGet;
  const { id } = useParams();
  const filterdata = areaofexpertiseAll.data.find((item) => item._id === id);
  useEffect(() => {
    if (filterdata) {
  ...
  }, [filterdata, areaofexpertiseAll.data, image]);

  const dispatch = useDispatch();
  dispatch(deleteareaofexpertiseEmpty());
  const areaofexpertiseupdat = useSelector(
    (state) => state.areaofexpertiseUpdate
  );
  const { error, areaofexpertiseUpdate } = areaofexpertiseupdat;
  const submitForm = (e) => {
    e.preventDefault();
    setLoading(true);
    let formData = {
    ...
    if (image) {
      ...
      uploadTask.on("state_changed", () => {
        getDownloadURL(uploadTask.snapshot.ref).then((url) => {
          formData.imgUrl = url;
          dispatch(updateareaofexpertise(formData, id))
            .then(() => {
  ...
  return (
    <>
      <Supercomponent>
        {areaofexpertiseUpdate && areaofexpertiseUpdate.success
