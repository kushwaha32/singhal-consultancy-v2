...
import {
  ...
  getassociates,
  updateassociatesEmpty,
} from "../../actions/associatesAction";

const Associates = () => {
  const dispatch = useDispatch();
  const associatesDAta = useSelector((state) => state.associatesData);
  const { loading, error, associatesAll } = associatesDAta;
  const associatesRemove = useSelector((state) => state.associatesDelete);
  const { associatesDelete } = associatesRemove;
  const onDelete = (id) => {
    Swal.fire({
      title: "Are you sure?",
      ...
      if (result.isConfirmed) {
        dispatch(deleteassociates(id)).then(() => {
  ...
  useEffect(() => {
    dispatch(getassociates());
    dispatch(updateassociatesEmpty());
  ...
  return (
    <>
      {associatesDelete && associatesDelete.success
        ? suErr("success", "Resource removed successfully")
        : associatesRemove.error && associatesRemove.error.success === false
                      ...
                      {associatesAll.data?.map((el, i) => {
                        return (
                          <tr key={i}>
                            ...
                            <td>
                              <Link
                                to={`/admin/associates/edit/${el._id}`}
