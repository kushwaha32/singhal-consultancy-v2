...
const PartnersEdit = () => {
  ...
  const partnersGet = useSelector((state) => state.partnersData);
  const { partnersAll } = partnersGet;
  const { id } = useParams();
  const filterdata = partnersAll.data.find((item) => item._id === id);
  useEffect(() => {
    if (filterdata) {
  ...
  }, [filterdata, partnersAll.data, image]);

  const dispatch = useDispatch();
  dispatch(deletepartnersEmpty());
  const partnersupdat = useSelector((state) => state.partnersUpdate);
  const { error, partnersUpdate } = partnersupdat;
  const submitForm = (e) => {
    e.preventDefault();
    setLoading(true);
    let formData = {
    ...
    if (image) {
      ...
      uploadTask.on("state_changed", () => {
        getDownloadURL(uploadTask.snapshot.ref).then((url) => {
          formData.imgUrl = url;
          dispatch(updatepartners(formData, id))
            .then(() => {
  ...
  return (
    <>
      <Supercomponent>
        {partnersUpdate && partnersUpdate.success
