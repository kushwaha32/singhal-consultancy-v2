...
const InformationtechnologyEdit = () => {
  ...
  const informationtechnologyGet = useSelector(
    (state) => state.informationtechnologyData
  );
  const { informationtechnologyAll } = informationtechnologyGet;
  const { id } = useParams();
  const filterdata = informationtechnologyAll.data.find(
    (item) => item._id === id
  );
  useEffect(() => {
  ...
  }, [filterdata, informationtechnologyAll.data, image]);

  const dispatch = useDispatch();
  dispatch(deleteinformationtechnologyEmpty());
  const informationtechnologyupdat = useSelector(
    (state) => state.informationtechnologyUpdate
  );
  const { error, informationtechnologyUpdate } = informationtechnologyupdat;
  const submitForm = (e) => {
    e.preventDefault();
    setLoading(true);
    let formData = {
    ...
    if (image) {
      ...
      uploadTask.on("state_changed", async () => {
        getDownloadURL(uploadTask.snapshot.ref).then((url) => {
          formData.imgUrl = url;
          dispatch(updateinformationtechnology(formData, id))
            .then(() => {
  ...
  return (
    <>
      <Supercomponent>
        {informationtechnologyUpdate && informationtechnologyUpdate.success
...
export default InformationtechnologyEdit;
